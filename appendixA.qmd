...
```{r}
x <- lx%>%filter(sex=="female")%>%select(x=age_group)
lx_f <- lx%>%filter(sex=="female")%>%select(lx=value)
lx_m <- lx%>%filter(sex=="male")%>%select(lx=value)
...
infectious::Glifetables %>%
  distinct()%>%
  filter(indicator=="nLx - person-years lived between ages x and x+n",
         year=="2019",
         sex=="female")
```
